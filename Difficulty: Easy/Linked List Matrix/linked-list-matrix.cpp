...
class Solution {
  public:
    Node* constructLinkedMatrix(vector<vector<int>>& mat) {
        ...
        // 09.10.24 POTD
        int n=mat.size();
        vector<vector<Node*>> linkedMat(n, vector<Node*>(n, NULL));
        
        for(int i=n-1; i>=0; i--){
            for(int j=n-1; j>=0; j--){
                Node *nd = new Node(mat[i][j]);
                if(j<n-1)   nd->right = linkedMat[i][j+1];
                if(i<n-1)   nd->down = linkedMat[i+1][j];
                linkedMat[i][j] = nd;
            }
        }
        
        return linkedMat[0][0];
    }
};
