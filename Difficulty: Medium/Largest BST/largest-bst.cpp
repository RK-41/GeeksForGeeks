...
class Solution{
    ...
    // bool dft(Node *nd, Node* parent, int& ans, int dir, int count){
    //     if(!nd) return true;
        
    //     bool left = dft(nd->left, nd, ans, -1, count);
    //     bool right = dft(nd->right, nd, ans, 1, count);
        
    //     if(left && right){
    //         ans = max(ans, count+1);
    //     }
        
    //     if(dir == -1 && nd->data < parent->data)
    //         return true;
